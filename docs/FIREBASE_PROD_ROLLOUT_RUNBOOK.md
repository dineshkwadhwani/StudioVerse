...
- Four root causes:
  ...
  4. `CoinRequestsModal` called `approveCoinRequest`/`denyCoinRequest` with the literal string `"company"` as `approvedBy`/`deniedBy`. That string ended up as `createdBy` on the wallet transactions, failing the `isActorId(createdBy)` check.
...
- `src/modules/wallet/components/CoinRequestsModal.tsx` — `approveCoinRequest` and `denyCoinRequest` now pass `auth.currentUser?.uid` instead of the literal `"company"`. Required for the `isActorId(createdBy)` check on `walletTransactions` create.

**Code change requires app rebuild/redeploy** (not just rules deploy) for the modal fix to take effect.
