/**
 * P-EVT-001 — Coach (Shilpa) creates an Event.
 */
...
const TEST_NAME = "Coach E2E Test Event";
const COIN_IMAGE = path.resolve(
  __dirname,
  "../../public/tenants/coaching-studio/coin.png"
);

test.describe("Coach · Manage Events · Create Event", () => {
  test.beforeEach(async () => {
    await deleteDocsWhere("events", "name", TEST_NAME);
  });

  test("Coach creates a draft Event (coin.png thumbnail)", async ({ page }) => {
    ...
    await page.fill("#event-short-description", "Short description for Coach e2e test event.");
    await page.fill("#event-long-description", "Long description for the Coach e2e test event.");
