/**
 * C-B3 — Company (Narendra) assigns a Program to a Cohort.
 *
 * Setup (beforeAll):
 *   • Bootstrap a published + listing-attached Program (so it shows on the
 *     /assign-activity feed which filters to isPublishedPublic + listing).
 *   • Temporarily associate Kartik to Narendra so he is a valid cohort member.
 *   • Bootstrap a cohort with Shilpa as Professional + Kiran + Kartik as
 *     members (status="active": memberCount>=2 AND professionalId set).
 *   • Top up Narendra's wallet with enough credits for 2 members × 50 cost.
 *
 * Flow:
 *   1. Narendra signs in → /coaching-studio/assign-activity.
 *   2. Programs tab → click "Find Out More" on the target program.
 *   3. In DetailModal click "Assign" → opens AssignmentModal.
 *   4. In AssignmentModal click the "Cohort" target tab.
 *   5. Select the bootstrapped cohort from the dropdown → Continue.
 *   6. On the confirm stage click "Assign".
 *
 * Verifies:
 *   • A new `cohortAssignments/` doc exists for cohort+activity.
 *   • Two `assignments/` docs exist (one per member) referencing the
 *     cohortAssignmentId.
 *   • Narendra's wallet was debited by perMemberCost × memberCount.
 *
 * Cleanup:
 *   • Revert Kartik's association.
 *   • Delete bootstrapped cohort/members/program/listing-package and any
 *     cohortAssignments / assignments / walletTransactions tied to the run.
 */
...
const PROGRAM_NAME = "Company Cohort Assign Target";
const LISTING_PACKAGE_NAME = "Company Cohort Assign Listing";
const COHORT_NAME = "E2E Cohort Assign Target";
...
test.describe("Company · Assign Activity · Assign Program to Cohort", () => {
  test.beforeAll(async () => {
    ...
    // cohort lookup (and any membership reads) treat him as in-scope.
    await setUserAssociatedCompany({
      userId: kartikUserId,
      associatedCompanyId: companyUserId,
    });

    // Published + listing-attached program (Assign Activities filter rule).
    ...
    // Fund Narendra: needs >= PER_MEMBER_COST × MEMBER_COUNT.
  ...
  test("Company assigns a Program to a Cohort via the Cohort tab", async ({ page }) => {
    ...
    const before = await getWalletStateForUser(companyUserId);
    ...
    // The cohort dropdown should populate from listCohortsForScope.
    const cohortSelect = modal.locator("#cohortSelect");
    await expect(cohortSelect).toBeVisible({ timeout: 15_000 });
    await expect
      .poll(async () => {
        const optionCount = await cohortSelect.locator("option").count();
        return optionCount;
      }, { timeout: 15_000 })
      .toBeGreaterThan(1);
    ...
    // Verify wallet debit.
    const after = await getWalletStateForUser(companyUserId);
